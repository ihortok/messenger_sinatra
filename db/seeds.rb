...
chat = Chat.find_or_initialize_by(name: 'Book talks')

chat.save! unless chat.persisted?

User.all.each do |user|
  ChatMember.create(chat: chat, user: user, admin: user.nickname == 'test')

  Message.create(sender: user, chat: chat, content: Faker::Lorem.sentence)
